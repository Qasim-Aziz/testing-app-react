...
const TargetsAvailableDrawer = ({
  ...
  const getCategory = async () => {
    ...
    if (notNull(response)) {
      setCategory(response.data.peakEquDomains)
      setSelectedCategory(response.data.peakEquDomains[0].name)
      setLoading(true)
    }
  }
  ...
  return (
    <>
      <div>
        <Radio.Group value={selectedCategory} onChange={e => selectCategory(e.target.value)}>
          {category.map(item => (
        ...
        <Input
          ...
          style={{ width: '300px', marginLeft: '20px', float: 'right' }}
        />
      </div>
      ...
      <Row>
        ...
        <Col md={12} style={{ padding: '10px' }}>
          <div style={cardsDivStyle}>
            {searchedTargets && searchedTargets.length > 0 ? (
              <>
