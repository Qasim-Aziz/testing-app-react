...
class PeakEqvi extends React.Component {
    ...
    componentDidMount() {
        const { dispatch, peakequivalence: { ProgramId } } = this.props
    ...
    goToPreviousTrial = () => {
        ...
        dispatch({
            ...
            payload: {
                ...
                SelectedTestId: PEQuestionsListObject[SelectedDomainId][SelectedQuestionIndex]?.node.test.edges[SelectedTestIndex - 1]?.node.id
    ...
    goToPreviousQuestion = () => {
        ...
        dispatch({
            ...
            payload: {
                ...
                SelectedQuestionId: PEQuestionsListObject[SelectedDomainId][SelectedQuestionIndex - 1]?.node.id,
                SelectedTestIndex: 0,
                SelectedTestId: PEQuestionsListObject[SelectedDomainId][SelectedQuestionIndex - 1]?.node.test.edges[0]?.node.id,
            }
        })
    ...
    recordResponse = response => {
        ...
        dispatch({
            ...
            payload: {
                ...
                testId: PEQuestionsListObject[SelectedDomainId][SelectedQuestionIndex]?.node.test.edges[SelectedTestIndex]?.node.id,
                response: response
            }
        })
    }
    ...
    checkDomainStatus = domainId => {
        const { peakequivalence: { PEQuestionsListObject, ResponseObject } } = this.props
                                            ...
                                            <>
                                                <div>
                                                    <Text style={{ fontSize: '18px', lineHeight: '24px' }}>
                                                        {PEQuestionsListObject[SelectedDomainId][SelectedQuestionIndex]?.node.questionText}
                                                    </Text>
                                                    ...
                                                    <span style={{ float: 'right' }}>
                                                        ...
                                                        &nbsp; Question {SelectedQuestionIndex + 1} / {PEQuestionsListObject[SelectedDomainId]?.length} &nbsp;
                                                            {SelectedQuestionIndex + 1 === PEQuestionsListObject[SelectedDomainId]?.length ?
                                                            <Button disabled>
                                                                <Icon type="right" />
                                                            </Button>
                                                            :
                                                ...
                                                {PEQuestionsListObject[SelectedDomainId][SelectedQuestionIndex]?.node.test.edges.length > 0 && (
                                                    <div
                                                        style={{
                                                            background: '#FFFFFF',
                                                            border: '1px solid #E4E9F0',
                                                    ...
                                                    >
                                                        ...
                                                        <Title style={titleStyle1}>{PEQuestionsListObject[SelectedDomainId][SelectedQuestionIndex]?.node.test.edges[SelectedTestIndex]?.node.name} </Title>
                                                        <div style={{ textAlign: 'right', marginTop: '20px' }}>
                                                            <Button onClick={() => this.recordResponse(true)} style={ResponseObject[SelectedQuestionId][SelectedTestId].recorded && ResponseObject[SelectedQuestionId][SelectedTestId].response === true ? buttonTrueStyle : buttonDefaultStyle}>{StudentName} gave an expected response</Button>
                                                            <br style={{marginTop: 20}} />
                                                            <Button onClick={() => this.recordResponse(false)} style={ResponseObject[SelectedQuestionId][SelectedTestId].recorded && ResponseObject[SelectedQuestionId][SelectedTestId].response === false ? buttonFalseStyle : buttonDefaultFalseStyle}>{StudentName} gave an unexpected response</Button>
                                                        </div>
                                                ...
                                                <div style={{ textAlign: 'right', marginTop: '50px' }}>
                                                    <span style={{ float: 'right' }}>
                                                        ...
                                                        &nbsp;Test {SelectedTestIndex + 1} / {PEQuestionsListObject[SelectedDomainId][SelectedQuestionIndex]?.node.test.edges.length}&nbsp;
                                                        {SelectedTestIndex + 1 === PEQuestionsListObject[SelectedDomainId][SelectedQuestionIndex]?.node.test.edges.length ?
                                                            <Button disabled>
                                                                <Icon type="right" />
                                                            </Button>
                                                            :
                                ...
                                <Col sm={8}>
                                    ...
                                    >
                                        <div style={{ height: '600px', overflow: 'auto' }}>
                                            {/* <Radio.Group onChange={(e) => this.LoadAssessment(e.target.value)} style={{ width: '100%' }} buttonStyle={{ width: '33.33%' }} defaultValue={SelectedPeakType}>
                                                    <Radio.Button style={{ width: '33.33%', textAlign: 'center' }} value="Basic">Basic</Radio.Button>
                                                    <Radio.Button style={{ width: '33.33%', textAlign: 'center' }} value="Intermediate">Intermediate</Radio.Button>
                                                    <Radio.Button style={{ width: '33.33%', textAlign: 'center' }} value="Advanced">Advanced</Radio.Button>
                                                </Radio.Group> */}
                                            {Loading ?
                                                <p>Loading...</p>
                                                :
                                                ...
                                                <Collapse accordion bordered={false} onChange={(e) => this.changeActiveDomian(e)} activeKey={SelectedDomainId} key={SelectedDomainId}>
                                                    {PEDomainList.map((item, index) =>
                                                        ...
                                                        <Panel header={`${item.name} (${this.checkDomainStatus(item.id)}) `} key={item.id}>
                                                            {PEQuestionsListObject[item.id].map((node, nodeIndex) =>
                                                                <>
                                                                    <div onClick={() => this.changeQuestion(nodeIndex)} style={node.node.id === SelectedQuestionId ? selectedCardStyle : cardStyle}>
                                                                        <Title style={node.node.id === SelectedQuestionId ? selectedTitleStyle : titleStyle}>{node.node.questionText}</Title>
                                                                    </div>
                                                                </>
                                                            )}
                                                        </Panel>
                                                    )}
                                                    {/* <Panel header="Report" key="report">
                                                        &nbsp;
                                                    </Panel> */}
