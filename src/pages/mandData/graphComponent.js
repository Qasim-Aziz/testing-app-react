...
const MAND_INFO = gql`
  query mandReport($studentId: ID!, $mandId: ID!, $sdate: Date!, $edate: Date!) {
...
const GraphComponent = ({ mandId, studentId, date }) => {
  ...
  const gdata2 = [
    { id: data ? data.mandReport[0]?.measurments : '', data: data ? data.mandReport[0]?.data : [] },
  ]
    ...
    <div className="widget" style={{ minHeight: 500, height: 500 }}>
      ...
      <RangePicker
        ...
        onCalendarChange={value => {
          changeDatesData(value)
        }}
        style={{ paddingLeft: 40 }}
      />
      {gdata2[0].data?.length > 0 && (
        <ResponsiveLine
          key={gdata2[0].data.length}
          data={gdata2}
