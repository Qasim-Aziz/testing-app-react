...
class StudentDrawer extends Component {
    ...
    handleSubmit = e => {
        ...
        form.validateFields((error, values) => {
            ...
            if (!error) {
                apolloClient.mutate({
                    mutation: gql`mutation UpdateEquivalenceTarget(
                        $targetId: ID!
                        $targetName: String!
                        $maxSd: Int!
                        $targetIns: String
                    ...
                    {selectedAlpha.map((alpha, alphaIndex) => (
                        ...
                        >
                            {getFieldDecorator(`stimulus${k}${alpha}`, {
                                initialValue: selectedCodeNode?.classes?.edges[k]?.node.stimuluses.edges[alphaIndex]?.node.stimulusName,
                ...
                {selectedCodeNode && (
                    <Form name="targetForm" onSubmit={this.handleSubmit}>
                        ...
                        <Form.Item label="Number of stimulus per class">
                            ...
                            })(<Input placeholder="Number of stimulus" size="large" type="number" max={6} min={2} />)}
                        </Form.Item>
