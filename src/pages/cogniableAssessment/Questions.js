...
class Questions extends React.Component {
  ...
  movePrev = () => {
    ...
    const {
      dispatch,
      cogniableassessment: { Question, AssessmentObject, QuestionCounter },
    } = this.props
  ...
  recordResponse = id => {
    const {
      dispatch,
      cogniableassessment: { Question, AssessmentObject, QuestionCounter, ResponseObject },
    } = this.props
    if (ResponseObject[Question.id]?.recorded) {
      ...
      document.getElementsByClassName('responseButtons').forEach(ele => {
  ...
  continueAssessment = () => {
    const {
      dispatch,
      cogniableassessment: { Question, AssessmentObject, QuestionCounter, cloneQuestion, isEdit },
    } = this.props
    dispatch({
      type: 'cogniableassessment/SET_STATE',
      payload: {
  ...
  enableEdit = () => {
    ...
    dispatch({
      ...
      payload: {
        Question:
          AssessmentObject.assessmentQuestions.edges[
            AssessmentObject.assessmentQuestions.edges.length - 1
          ].node.question,
        isEdit: true,
        // QuestionCounter: AssessmentObject.assessmentQuestions.edges.length
      },
      ...
      return (
        <>
          ...
          >
            ...
            <Text style={titleStyle}>You answered all the cogniAble assessment questions</Text>{' '}
            <br />
            <Button style={{ height: 50, marginTop: '20px' }} onClick={this.saveQuestionAnswer}>
              Move to the next segment
            </Button>
            <br />
            <Button style={{ marginTop: '20px' }} type="link" onClick={this.enableEdit}>
    ...
    return (
      <>
        <Text style={titleStyle}>
          {AssessmentObject.student.firstname}&apos;s CogniAble Assessment
        </Text>
                    ...
                    {ResponseObject[Question.id]?.response?.answer.id === item.node.id ? (
                      <Col span={12}>
                        <Button
                          ...
                          onClick={() => this.recordResponse(item.node.id)}
                        >
          ...
          {isEdit ? (
            <div style={{ textAlign: 'right', marginTop: '40px', marginRight: '20px' }}>
              {QuestionCounter > 1 ? (
                ...
                <>
                  <Button disabled onClick={this.movePrev}>
                    <Icon type="left" />
                  </Button>
                  &nbsp;&nbsp;
                </>
              ...
              {QuestionCounter < AssessmentObject.assessmentQuestions.edges.length ? (
                ...
                <>
                  <Button disabled onClick={this.moveNext}>
                    <Icon type="right" />
                  </Button>
                </>
              )}
