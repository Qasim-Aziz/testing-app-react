...
export async function getData(payload) {
  return apolloClient
    .query({
      query: gql`
        query GetIISAData($studentId: ID!) {
...
export async function createAssessment(payload) {
  return apolloClient
    .mutate({
      mutation: gql`
        mutation CreateAssessment($studentId: ID!, $title: String!, $note: String, $date: Date!) {
          IISACreateAssessment(
            input: { student: $studentId, title: $title, date: $date, notes: $note }
          ) {
            details {
              id
              title
...
export async function recordResponse(payload) {
  return apolloClient
    .mutate({
      mutation: gql`
        mutation RecordResponse($assessmentId: ID!, $questionId: ID!, $optionId: ID!) {
          IISARecording(
            input: { pk: $assessmentId, record: [{ question: $questionId, option: $optionId }] }
          ) {
            responses {
              id
              question {
    ...
    .catch(error => {
      console.log('ERROR ====> ', error)
      return notification.error({
        message: 'Something went wrong recording response',
        description: 'please check console for Error message',
      })
    })
}
    ...
    .mutate({
      mutation: gql`
        mutation EndAssessment($objectId: ID!) {
          IISAUpdateAssessment(input: { pk: $objectId, isActive: false }) {
            details {
              id
              title
              date
...
export async function getAssessmentReport(payload) {
  return apolloClient
    .mutate({
      mutation: gql`
        mutation AssessmentSummary($objectId: ID!) {
          IISAAssessmentSummary(input: { pk: $objectId }) {
            score
            classification
            details {
              id
    ...
    .catch(error => {
      console.log('ERROR ====> ', error)
      return notification.error({
        message: 'Something went wrong loading Summary',
        description: 'please check console for Error message',
      })
    })
}
